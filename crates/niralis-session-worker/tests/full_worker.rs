...
impl FullWorker {
    ...
    fn spawn_process(mode: &str, with_control: bool) -> Self {
        ...
        let harness_fd = child_harness.as_raw_fd();
    ...
    fn send_request(&mut self) {
        ...
        let stdin = self.stdin.as_mut().expect("worker stdin remains connected");
    ...
    fn acknowledge_scope(&self, registration_nonce: &str) {
        let mut stream = UnixStream::connect(&self.control_path).expect("connect ack channel");
    ...
    fn answer_release(&mut self, recovery: bool) {
        ...
        let mut stream = UnixStream::connect(&self.control_path).expect("connect release channel");
